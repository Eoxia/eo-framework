...
class Update_Manager_Action {
	...
	public function callback_admin_scripts() {
		wp_enqueue_style( 'wpeo_update_manager_style', \eoxia\Config_Util::$init['eo-framework']->wpeo_update_manager->url . '/assets/css/style' . ( DEBUG ? '' : '.min' ) . '.css', array() );
		wp_enqueue_script( 'wpeo_upload_script', \eoxia\Config_Util::$init['eo-framework']->wpeo_update_manager->url . '/assets/js/wpeo-update-manager.js', array( 'jquery' ), \eoxia\Config_Util::$init['eo-framework']->wpeo_update_manager->version );
	}
